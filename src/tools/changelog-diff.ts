...
const CLASSIFICATION_RULES: { category: ChangelogEntry['category']; patterns: RegExp[] }[] = [
...
export class ChangelogParser {
     ...
     */
    private async discoverChangelog(library: LibraryEntry): Promise<string> {
        ...
        const promises = CHANGELOG_PATHS.map(filePath =>
            this.fetcher.fetchDocString(library, filePath, 'main')
        );
     ...
     */
    private async fetchFromGitHubReleases(library: LibraryEntry): Promise<string> {
        const repoMatch = library.repository.match(/github\.com\/([^/]+)\/([^/]+)/);
        if (!repoMatch) throw new Error(`No changelog found for ${library.id}`);
     ...
     */
    private parseVersionSections(rawText: string): Map<string, string[]> {
     ...
     */
    private classifyEntry(text: string): ChangelogEntry['category'] {
    ...
    private getVersionRange(sections: Map<string, string[]>, fromVersion: string, toVersion: string): string[] {
        ...
        if (toIdx === -1) {
            if (fromIdx === -1) return allVersions;
     ...
     */
    public async getDiff(library: LibraryEntry, fromVersion: string, toVersion: string): Promise<ChangelogDiff> {
        // Stage 1: Discover
            ...
            for (const bv of library.metadata.breakingVersions) {
                if (this.isVersionInRange(bv, fromVersion, toVersion)) {
                    breakingChanges.push(
                        `CRITICAL: Version ${bv} is a known major breaking change boundary. Check official migration guide.`
                    );
                }
            }
        }
