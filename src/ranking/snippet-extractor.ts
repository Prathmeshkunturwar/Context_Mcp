...
export class SnippetExtractor {
    ...
    public extractChunks(content: string, fileName: string): DocChunk[] {
    ...
    private extractJupyterChunks(rawJson: string, sourcePath: string): DocChunk[] {
        const chunks: DocChunk[] = [];
        try {
            const notebook = JSON.parse(rawJson);
            if (!notebook.cells) return chunks;
